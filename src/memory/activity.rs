// Runtime activity types lifted from crates/jcode-memory-types/src/lib.rs (Ola 2 Agent 3 - MemoryActivityLift).
// Per exact proposal in ORCHESTRATION_STATUS.md (Ola 1 Agent F verification) and Fase0_Baseline_Report
// "Memory types purity violation" section: these (~198 LOC) are runtime state machines + std::time::Instant
// (not plain serializable data contracts). Now canonical owner is this monolith module; jcode-memory-types
// reduced to pure MemoryEntry/MemoryGraph + ranking/score/normalize fns only. Thin re-export shim
// maintained in src/memory_types.rs for untouched TUI/compat sites. Do not move snapshot fns or protocol.
...
/// One clean test for the lifted activity types (Ola 2 Agent 3).
#[cfg(test)]
mod tests {
    use super::*;
    use std::time::Instant;
    ...
    #[test]
    fn pipeline_state_new_is_pending_and_not_complete() {
        let p = PipelineState::new();
        assert_eq!(p.search, StepStatus::Pending);
        assert_eq!(p.verify, StepStatus::Pending);
        assert!(!p.is_complete());
        assert!(!p.has_running_step());
    }

    #[test]
    fn pipeline_completes_when_all_steps_done() {
    ...
    #[test]
    fn memory_activity_is_processing_when_not_idle() {
