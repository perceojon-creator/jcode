...
// Runtime activity types (MemoryActivity etc) lifted to monolith src/memory/activity.rs (Ola 2 Agent 3).
// jcode-memory-types now contains ONLY plain serializable data contracts + pure fns per Fase0
// Baseline + Ola 1 Agent F proposal. See src/memory_types.rs for thin re-export shim (compat only).
...
/// Internal ID generator seam for MemoryEntry.
/// Duplicates the tiny logic from jcode-core::id (chrono timestamp + rand)
/// so that jcode-memory-types can remain a pure data-contract crate with
/// no dependency on jcode-core (per CRATE_OWNERSHIP_BOUNDARIES.md and
