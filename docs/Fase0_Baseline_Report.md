...
|---|-------------|------------|--------------------------------------|
| 1 | Server Service Handles (zero new crates) | 95% | 5 handles + `ServerServices` bag fully wired (Ola 1). **Ola 2 (Agent 1 + Agent 2)**: signatures narrowed 29 → 2/3 params; 47 direct swarm refs removed; 5 new SwarmServiceHandle methods. **Ola 3 Agent 1 (Move6-MonitorBusExtractor)**: monitor_bus + bg dispatch paths partially behind MaintenanceServiceHandle (thin delegates + run_monitor_bus impl on handle in server.rs:1354; background_tasks.rs:80-86 dispatch fns moved verbatim per SPLIT_PLAN Move 6); call sites updated. **Ola 3 VerifierIntegrator (Agent 5) + Ola 4 #1 Move6VerificationSupport gate (2026-05-29)**: +1% (95%); both profiles cargo check -p jcode --lib GREEN (40.08s default / 11.28s selfdev per verification_gate logs); test compile friction isolated but lib solid. Bag dominant conduit. Ready for Wave 4.1 full Move 6 collapse (target 98-99%). |
...
| 9 | Build/Compile Hygiene | 82% | Ola 1/2/3 + **Ola 4 #1 Move6VerificationSupport full gate (2026-05-29)**: +7%. Default check 40.08s exit 0 + selfdev 11.28s exit 0 (warm baselines in verification_gate_check_*.txt); boundary.py x2 PASS; 2802 test build RED (6 stale test-module refs: E0432 subscribe_should_mark_ready/dispatch_*, E0308 u32/u64 in streaming.rs, E0061 old 29-arg handle_client in lifecycle_tests — all pre-Ola 4 #1, lib unaffected). Ola 4 #1 stabilization delivered (lib + selfdev clean). Test debt quantified for Wave 4.1. See ORCHESTRATION Ola 4 #1 verifier block + root logs. |
